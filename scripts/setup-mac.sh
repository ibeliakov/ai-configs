...
# The shortcut API token is resolved from automation/.env into the generated (gitignored) file
# so it never enters git. Any stale symlink at the target is removed first.
...
sc = merged.get('shortcut')
if sc and isinstance(sc.get('env'), dict) and 'SHORTCUT_API_TOKEN' in sc['env']:
    token = env_value(env_path, 'SHORTCUT_API_TOKEN')
    if token:
        sc['env']['SHORTCUT_API_TOKEN'] = token
    else:
        sys.stderr.write("  warn: SHORTCUT_API_TOKEN not set in automation/.env - leaving placeholder in .mcp.json\n")
...
# $SHORTCUT_MEMBER_ID. Stays in the gitignored settings.local.json, never in git.
