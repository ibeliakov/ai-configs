...
# Project servers win on key conflict; common adds shortcut. The shortcut server is the official
# hosted HTTP server (OAuth) — no API token is embedded here.
...
# use them via $env:SHORTCUT_API_TOKEN / $env:SHORTCUT_MEMBER_ID. The hosted MCP server authenticates
# via OAuth (no token needed), but the token is still used by /shortcut-* to fetch story attachments
...
if ($token) {
  $json.env | Add-Member -MemberType NoteProperty -Name 'SHORTCUT_API_TOKEN' -Value $token -Force
}
...
if ($token)    { Write-Host "     SHORTCUT_API_TOKEN=(set)" }
