...
- Creating or editing Redux code → [Redux structure](.claude/rules/redux.md)
- Adding routes or working with SSR → [Routing & SSR](.claude/rules/routing.md)
...
- **Next.js** 13.0.5 — file-based routing, SSR via `getServerSideProps`
- **React** 18.2.0 — functional components only, `FC<Props>` typing
- **TypeScript** 4.9.3 — `strict: true`, use `type` not `interface`
- **Redux** — legacy only: `redux` + `redux-thunk`, switch-case reducers. No `@reduxjs/toolkit`
- **Ant Design** 3.26.5 — icons via `Icon` component, `Form.create()` HOC
...
- tsconfig target is `es2022`

## Imports

- Use path aliases: `@src/`, `@components/`, `@styles/`, `@pages/`, `@public/`
...
src/
  ...
  store/
    configureStore.ts   store bootstrap + next-redux-wrapper HYDRATE
...
Always use `useAppDispatch` — never raw `useDispatch`. For selectors, use `useSelector` typed with `RootState`.
...
- `ClientApi` is a JavaScript file — import as: `import ClientApi from '@src/helpers/ClientApi'`
- Feature-specific API functions live in `src/api/` — use those instead of calling ClientApi directly in components
...
pages/              Next.js routes (file-based routing)
  _app.tsx          Redux Provider, auth token sync, device tracking
  _document.tsx     HTML document customization
  404.tsx           custom 404
  index.tsx         career page (/)
  [slug]/           dynamic routes
...
src/
  ...
  constants/        app-wide constants, path strings
  helpers/
    ClientApi.js    Axios instance with auth headers (JS, not TS)
    forActions.ts   handleError helper
    storage.ts      localStorage helpers
    regexp.tsx      all project regex patterns
    rulesFileds.ts  AntD Form validation rule builders
    date.ts         date formatting utilities
    string.ts       string processing utilities
    formated.ts     formatting utilities
    object.ts       object utilities
    list.ts         list/array utilities
  hooks/            custom React hooks
  interfaces/       TypeScript types
  reducers/         legacy switch-case reducers
  selectors/        reselect selectors
  store/
    configureStore.ts store bootstrap + next-redux-wrapper
    slices/           domain reducers (legacy switch-case pattern)
```

## Folder responsibilities

**`components/templates/`** — reusable presentational components used across multiple features.
No business logic, no direct Redux access. Receive everything via props.

**`components/views/`** — feature-level components responsible for a complete piece of functionality.
Connect to Redux, contain business logic, compose templates components.

**`components/pages/`** — page-level shells. Wire up views, handle route params,
...
| Path | Description |
|---|---|
| `src/helpers/ClientApi.js` | Axios instance — JS file, edit only for auth/header changes |
| `src/helpers/Utils.js` | Legacy utility collection |
| `src/helpers/componentsWrappers.js` | Legacy component wrappers |
